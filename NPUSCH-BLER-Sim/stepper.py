...
class Worker(Thread):
    ...
    NIREP=8   # (0..7)
    NIRU=8    #(0..7)
    NISNR=501 #(0..500) #SNR = -30+ISNR*0
    
    def get_snr(self,isnr):
        -30+isnr*0.1
...
#    NISNR=121 #(0..100) #SNR = -35+ISNR*0    
    ...
    def process_result(self,result):    
        self._log.info( "Finished IMCS:%i, IRU:%i, IREP:%i, ISNR:%i, SNR:%2.1f, BLER=%.5f" % \
                       (result['IMCS'],result['IRU'],result['IREP'],result['ISNR'],result['snr'],result['bler']) )
        ...
        def storeIter(IREP_range,ISNR_range):                                        
            for xIREP in IREP_range:
                for xISNR in ISNR_range:
                   yield result['IMCS'],result['IRU'],xIREP,xISNR,1,result['bler'],self.get_snr(xISNR),result['tbs'],2**xIREP * self.NRU[int(result['IRU'])]
...
if __name__ == '__main__': 
   ...
   with ThreadPool(dbname,8,5) as pool:
       if not pool.resume():
          #hand picked seeds
