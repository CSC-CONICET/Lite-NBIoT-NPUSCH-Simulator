...
IREP = 0;                      % Repetition number field in DCI (DCI format N1 or N2)
...
% 'parfor' will default to the normal 'for' statement.
    ...
    if ~ isnan(bler(IMCSIndex,ISFIndex,snrIdx))
        continue
    end
    ...
    stitle=sprintf('Simulating at IMSC:%g ISF:%g %gdB SNR' ,IMCS(IMCSIndex),ISF(ISFIndex),SNR(snrIdx));
    %print stitle
    ...
    bler(IMCSIndex,ISFIndex,snrIdx) = numBlkErrors/numTrBlks;
    save('partial.mat','bler')
    toc
end
...
save('blerResults.mat','bler','IMCS','SNR','ISF')
